...
def valid_s(end_epoch,epoch_num,model,dataloader,device,ckpt,num_class,writer,logger,optimizer):
    ...
    with torch.no_grad():
        for batch_idx, (inputs, targets) in enumerate(loader):
            ...
            if type(outputs)==tuple:
                predictions = torch.round((torch.sigmoid(torch.squeeze(outputs[0], dim=1)) + torch.sigmoid(torch.squeeze(outputs[1], dim=1)) + torch.sigmoid(torch.squeeze(outputs[1], dim=1))) / 3.0)  
