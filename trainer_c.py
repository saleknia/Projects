...
def trainer(end_epoch,epoch_num,model,teacher_model,dataloader,optimizer,device,ckpt,num_class,lr_scheduler,writer,logger,loss_function):
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        temp = 4.0
        alpha = 0.1
        loss_disparity = alpha * (F.kl_div(F.log_softmax(outputs/temp, dim=1),F.softmax(outputs_t/temp, dim=1),reduction='batchmean') * temp * temp)
...
class RKD(nn.Module):
	...
	def __init__(self, w_dist=250, w_angle=500):
