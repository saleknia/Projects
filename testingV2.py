    ...
    if len(image.shape) == 3:
        ...
        for ind in range(image.shape[0]):
            ...
            if x != patch_size[0] or y != patch_size[1]:
                ...
                slice = cv2.resize(slice, None, fx=(patch_size[0] / x), fy=( patch_size[1] / y), interpolation = cv2.INTER_LINEAR)
            ...
            with torch.no_grad():
                ...
                if x != patch_size[0] or y != patch_size[1]:
                    ...
                    pred = cv2.resize(out, None, fx=(x / patch_size[0]), fy=(y / patch_size[1]), interpolation = cv2.INTER_LINEAR)
                else:
                    pred = out
                prediction[ind] = pred
    else:
...
def inferenceV2(model,logger,test_save_path=None,num_classes=9,img_size=256):
