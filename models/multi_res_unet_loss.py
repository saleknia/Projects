...
class Multiresblock(nn.Module):
  def __init__(self,input_features : int, corresponding_unet_filters : int ,alpha : float =1.67)->None:
    ...
    self.conv2d_bn_1x1 = Conv2d_batchnorm(input_features=input_features,num_of_filters = int(self.W*0.167)+int(self.W*0.333)+int(self.W*0.5),kernel_size = (1,1),activation='None',padding = 0)
...
class MultiResUnet_loss(nn.Module):
  def __init__(self,channels : int=1,filters : int =32,nclasses : int =9)->None:
    ...
    self.conv_final = Conv2d_batchnorm(input_features=self.in_filters9,num_of_filters = self.nclasses,kernel_size = (1,1),activation='None')
  ...
  def forward(self,x : torch.Tensor)->torch.Tensor:
    ...
    if self.training:
        ...
        return conv_final_layer, x_multires6, x_multires7, x_multires8, x_multires9
    else:
        if self.nclasses > 1:
            conv_final_layer =  self.conv_final(x_multires9)
        else:
