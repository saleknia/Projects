...
class DABModule(nn.Module):
    def __init__(self, nIn, d=1, kSize=3, dkSize=3):
        ...
        self.conv3x3 = Conv_DAB(nIn, nIn // 2, kSize, 1, padding=1, bn_acti=True)

        self.dconv3x1 = Conv_DAB(nIn // 2, nIn // 2, (dkSize, 1), 1,
                             padding=(1, 0), groups=nIn // 2, bn_acti=True)
        self.dconv1x3 = Conv_DAB(nIn // 2, nIn // 2, (1, dkSize), 1,
                             padding=(0, 1), groups=nIn // 2, bn_acti=True)
        self.ddconv3x1 = Conv_DAB(nIn // 2, nIn // 2, (dkSize, 1), 1,
                              padding=(1 * d, 0), dilation=(d, 1), groups=nIn // 2, bn_acti=True)
        self.ddconv1x3 = Conv_DAB(nIn // 2, nIn // 2, (1, dkSize), 1,
...
class DownSamplingBlock(nn.Module):
    def __init__(self, nIn, nOut):
        ...
        self.conv3x3 = Conv_DAB(nIn, nConv, kSize=3, stride=2, padding=1)
...
class DABNet(nn.Module):
    def __init__(self, block_1=3, block_2=6):
        ...
        self.init_conv = nn.Sequential(
            Conv_DAB(3, 32, 3, 2, padding=1, bn_acti=True),
            Conv_DAB(32, 32, 3, 1, padding=1, bn_acti=True),
            Conv_DAB(32, 32, 3, 1, padding=1, bn_acti=True),
        )
