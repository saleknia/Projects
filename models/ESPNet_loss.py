...
class ESPNet_loss(nn.Module):
    ...
    def __init__(self, num_classes=19, p=2, q=3, encoderFile=None):
        ...
        self.combine_l2_l3 = nn.Sequential(BR(2 * num_classes),
                                           DilatedParllelResidualBlockB(2 * num_classes, 5, add=False))

        self.up_l2 = nn.Sequential(
            nn.ConvTranspose2d(5, num_classes, 2, stride=2, padding=0, output_padding=0, bias=False), BR(num_classes))
    ...
    def forward(self, input):
        ...
        if self.training:
            return classifier, output2, output1, concat_features, output0, None, None, None, None, None
        else:
            return classifier