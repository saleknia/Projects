...
def trainer_s(end_epoch,epoch_num,model,dataloader,optimizer,device,ckpt,num_class,lr_scheduler,writer,logger,loss_function):
    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        with torch.autocast(device_type=device, dtype=torch.float16):
            ...
            if type(outputs)==tuple:
                loss_ce = (ce_loss(outputs[0], targets.unsqueeze(dim=1)) + ce_loss(outputs[1], targets.unsqueeze(dim=1)) + ce_loss(outputs[2], targets.unsqueeze(dim=1)) + ce_loss(outputs[3], targets.unsqueeze(dim=1)) + ce_loss(outputs[4], targets.unsqueeze(dim=1))) / 5.0
                loss_dice = (dice_loss(inputs=outputs[0], targets=targets) +  dice_loss(inputs=outputs[1], targets=targets) +  dice_loss(inputs=outputs[2], targets=targets) +  dice_loss(inputs=outputs[3], targets=targets) +  dice_loss(inputs=outputs[4], targets=targets)) / 5.0  
                loss_att = 0.0
        ...
        lr_ = 5e-5 * (1.0 - iter_num / max_iterations) ** 0.9
    ...
    #     optimizer.step()
        ...
        if type(outputs)==tuple:
            predictions = torch.round(torch.sigmoid(torch.squeeze(outputs[4], dim=1)))
