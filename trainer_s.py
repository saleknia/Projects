    ...
    for batch_idx, (inputs, targets) in enumerate(loader):
        ...
        with torch.autocast(device_type=device, dtype=torch.float16):
            ...
            if type(outputs)==tuple:
                ...
                loss_dice = dice_loss(inputs=outputs[0], targets=targets) + dice_loss(inputs=outputs[1], targets=targets) + dice_loss(inputs=outputs[2], targets=targets) + dice_loss(inputs=outputs[3], targets=targets) + dice_loss(inputs=outputs[4], targets=targets)
                loss_att = 0.0
